...
`;
const RECHERCHER_ETUDIANT = gql`
  query RechercherEtudiant($id: ID!) {
    rechercherEtudiant(id: $id) {
      id
      matricule
      nom
      prenom
...
rechercherEtudiant(id: string): Observable<Etudiant> {
  return this.etudiantClient
    .watchQuery<{ rechercherEtudiant: Etudiant }>({
      query: RECHERCHER_ETUDIANT,
      variables: { id },
    })
    .valueChanges.pipe(map(result => result.data.rechercherEtudiant));
}
